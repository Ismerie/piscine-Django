...
class Page:
    ...
    def write_to_file(self, path: str):
        try:
            ...
            print(f"Error: An error occurred while reading the template file '{path}': {e}")

    def is_valid(self):
        return self.__recursive_check(self.elem)

    def __recursive_check(self, elem: Elem()):
...
def __print_test(target: Page, toBe: bool):
    ...
    print(str(target))
    print("_______________IS_VALID_____________")
    assert target.is_valid() == toBe
    print("{:^36s}".format(str(target.is_valid())))
    print("________________END__________________")
    print()
